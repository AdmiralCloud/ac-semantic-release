...
const escapeSingleQuotes = (notes) => `${notes}`.replace("'", "\'")
  ...
  async.series({
    ...
    mergeAndPush: (done) => {
      ...
      exec('git add package.json CHANGELOG.md', (err) => {
        ...
        exec(`git commit -m '${escapeSingleQuotes(releaseMessage)}'`, (err) => {
          if (err) return done(err)

          exec(`git tag ${newTag} -m '${escapeSingleQuotes(releaseNotes)}'`, (err) => {
            if (err) return done(err)
