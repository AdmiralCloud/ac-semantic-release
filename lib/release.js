...
const release = () => {
  ...
  async.series({
      ...
      exec(command, (err, stdout) => {
        ...
        _.forEach(lines, line => {
          ...
          if (_.startsWith(line, 'commit')) {
            ...
            const date = _.trim(line.replace('Date: ', ''))
            ...
            if (!commit.body) {
              ...
              // Store that we encountered the Related issues line, but don't add text yet
              commit.hasRelatedIssues = true
              commit.relatedIssues = []
            }
            else if (_.startsWith(line, '-')) {
              // these are links to issues: https://github.com/ACCOUNT/REPO/issues/340
              const trimmedLine = _.trim(line.replace('-', ''))
              if (trimmedLine && !trimmedLine.includes('undefined')) {
                const elements = _.split(trimmedLine, '/')
                if (elements.length >= 5) {  // Ensure we have enough parts for a valid URL
                  const repo = _.get(elements, '[3]') + '/' + _.get(elements, '[4]')
                  if (repo && repo !== 'undefined/undefined') {
                    // If this is the first valid issue, add the header
                    if (commit.hasRelatedIssues && !commit.body.includes('Related issues:')) {
                      commit.body += 'Related issues: '
                    }
                    commit.body += '[' + repo + '#' + _.last(elements) + '](' + trimmedLine + ') '
                  }
                }
              }
            }
    ...
    checkRelease: (done) => {
      ...
      // if you want to set a fixed version (e.g. in case commit messages contain wrong information), use fixedVersion
      ...
      let changelog = ''
      const compiled = _.template(templateEntry)
      const placeholder = {
        release,
        version,
        repositoryUrl,
      ...
      _.forEach(commits, (messages, type) => {
        const compiled = _.template(templateSection)
        const prettyType = _.get(_.find(_.get(config, 'types'), { value: type }), 'changelog')
        changelog += compiled({ prettyType })
        releaseNotes += _.toUpper(prettyType) + '\n'

        // Commits 
        _.forEach(messages, commit => {
          const compiled = _.template(templateCommit)
      ...
      if (_.size(breakingChanges)) {
        ...
        _.forEach(breakingChanges, commit => {
          const compiled = _.template(templateBreaking)
          changelog += compiled(commit)
      ...
      fs.readFile(changelogFile, (err, result) => {
        ...
        const log = changelog + (result && result.toString())
    ...
    mergeAndPush: (done) => {
      ...
      exec('git add package.json CHANGELOG.md', (err) => {
        ...
        exec(`git commit -m "${escapeDoubleQuotes(releaseMessage)}"`, (err) => {
          if (err) return done(err)

          exec(`git tag ${newTag} -m "${escapeDoubleQuotes(releaseNotes)}"`, (err) => {
            if (err) return done(err)
