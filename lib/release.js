...
const release = async () => {
  ...
  // Step 0: optional Dependabot audit
  if (config.audit?.beforeRelease) {
    const { audit, report } = require('./audit')
    const result = await audit()
    report(result)
    if (result.failing.length && !debugMode) {
      console.log('Release aborted - fix the vulnerable packages above or add them to audit.ignore in .acsemver.js')
      process.exit(1)
    }
  }
...
release().then(() => { process.exitCode = 0 }).catch(err => { console.error(err); process.exitCode = 1 })
