...
const commit = async () => {
  ...
  const selectFiles = async ({ list }) => {
    ...
    const { stdout: cachedOut } = await exec('git diff --cached --name-only')
    const alreadyStaged = new Set(cachedOut.split('\n').filter(Boolean))
    const filesToAdd = files.filter(f => !alreadyStaged.has(f))
    if (filesToAdd.length > 0) {
      const addCommand = `git add -A -- ${filesToAdd.join(' ')}`
